...
class PointNetSetAbstraction_grid(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all:
            ...
            new_xyz, new_points, grouped_xyz_forgrid, _ = sample_and_group(self.npoint, self.radius, self.nsample, xyz, points, returnfps=True)
        # new_xyz: sampled points position data, [B, npoint, 3]
        # new_points: sampled points data, [B, npoint, nsample, C]
        # grouped_xyz: [B, npoint, nsample, 3]

        xyz_forgird = grouped_xyz_forgrid.permute(0, 3, 1, 2)
        # grid_feats = self.grid_render(new_points.permute(0, 3, 1, 2), grouped_xyz.permute(0, 3, 1, 2))
        grid_feats = self.grid_render(new_points.permute(0, 3, 1, 2), xyz_forgird)

        for i in range(len(self.sharedmlp3d)-1):
            grid_feats = self.mlp_convs_3d[i](grid_feats)
            grid_feats = F.relu(self.mlp_bns_3d[i](grid_feats))
            # print('grid_feats:',grid_feats.shape)
        new_features = F.max_pool3d(grid_feats, kernel_size=[grid_feats.size(2), grid_feats.size(3), grid_feats.size(4)])   #   (B*npoint, C, 1, 1, 1)
        new_features = new_features.reshape(new_points.size(0), new_points.size(1), -1).permute(0, 2, 1).contiguous()       #   (B, mlp[-1], npoint)
...
class LgrNet(nn.Module):
    def __init__(self,num_classes=40,normal_channel=False, **kwargs):
        ...
        self.sa1 = PointNetSetAbstraction_grid(npoint=512, radius=0.2, nsample=32, in_channel=in_channel, mlp=[64, 64, 128], group_all=False)
        self.sa2 = PointNetSetAbstraction_grid(npoint=128, radius=0.4, nsample=64, in_channel=128 + 3, mlp=[128, 128, 256], group_all=False)
